...
public class Game {
	...
	void collision() {
		for (int i = 0; i < entites.size(); i++) {
			if (entites.get(i).aabb.active) {
				for (int j = 0; j < entites.size(); j++) {
					if (entites.get(j).aabb.active) {
						if (entites.get(i).aabb.isColliding(entites.get(j).aabb)) {
							entites.get(i).collide(entites.get(j));
							entites.get(j).collide(entites.get(i));
						}
					}
				}
			}
